...
describe("saveEmail", () => {
  ...
  describe("validation", () => {
    it("rejects an invalid email format", async () => {
      expect(await saveEmail("not-an-email")).toEqual({
        ok: false,
        error: "Invalid email",
      });
    });
    ...
    it("rejects an empty string", async () => {
      expect(await saveEmail("")).toEqual({
        ok: false,
        error: "Invalid email",
      });
    });
  ...
  describe("missing session", () => {
    it("returns an error when the userId cookie is absent", async () => {
      ...
      expect(await saveEmail("user@example.com")).toEqual({
        ok: false,
  ...
  describe("database errors", () => {
    ...
    it("returns a generic error when the email update throws", async () => {
      ...
      const consoleSpy = vi
        .spyOn(console, "error")
        .mockImplementation(() => {});
      expect(await saveEmail("new@example.com")).toEqual({
        ok: false,
        error: "Failed to save email",
      });
      consoleSpy.mockRestore();
